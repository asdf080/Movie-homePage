    ...
    <section className='w-full flex justify-center'>
      <div id="trendWrap" className='w-[1300px] min-h-[450px] pt-[50px]'>
        ...
        <article style={{ backgroundImage: `url(${bgImage})` }} id="trendList" className='flex justify-center my-8 py-4'>
          ...
          : lists.slice(0, 7).map(item => (<div key={item.id} className='w-[150px] min-h-[293px] mt-4 mx-4 relative'>
            <Link to={`/detail/${item.id}`}>
              <img src={`https://www.themoviedb.org/t/p/w150_and_h225_face${item.poster_path}`} alt="img"className='rounded-2xl' />
              <div className='w-11 h-11 bg-gray-700 border-2 border-white p-1 rounded-full flex justify-center items-center absolute top-[195px] left-[-5px] font-semibold text-white'>{item.vote_average.toFixed(1)}</div>
